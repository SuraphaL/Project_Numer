...
class Backwardh2 extends Component {
    ...
    backwardh(x, h, x1) {
        ...
        y1 = (3 * this.func(x) - 4 * this.func(x - (1 * h)) + this.func(x - (2 * h))) / (2 * h)
        y1 = y1.toFixed(6);


        y2 = (2 * this.func(x) - 5 * this.func(x - (1 * h)) + 4 * this.func(x - (2 * h)) - this.func(x - (3 * h))) / Math.pow(h, 2)
        y2 = y2.toFixed(6);


        y3 = (5 * this.func(x) - 18 * this.func(x - (1 * h)) + 24 * this.func(x - (2 * h)) - 14 * this.func(x - (3 * h)) - this.func(x - (3 * h))) / (2 * Math.pow(h, 3))
        y3 = y3.toFixed(6);

        y4 = (3 * this.func(x) - 14 * this.func(x - (1 * h)) + 26 * this.func(x - (2 * h)) - 24 * this.func(x - (3 * h)) + 11 * this.func(x - (4 * h)) - 2 * this.func(x - (5 * h))) / Math.pow(h, 4)
        y4 = y4.toFixed(6);
    ...
    */
    func(X) {
        if (this.state.fx !== "") {
            ...
            let scope = { x: parseFloat(X.toFixed) }; //แปลงค่า string หรือตัวแปรให้เป็น Number
    ...
    render() {
        ...
        return (
            <div >
                <h2 style={{ marginTop: '24px', marginLeft: '30px', color: "black", fontWeight: "bold", fontSize: '24px' }}>Backward Divided-Differences Oh<sup>2</sup></h2>
                <div style={{ backgroundColor: '#fff1b8', padding: '30px' }}
                            ...
                            <Row>
                                <Col style={{ width: 400 }} span={12}>
                                    <Card style={{ width: 350, padding: '3px' }} title="Insert Eqation" bordered={false}>
                                        ...
                                        <Button onClick={() => this.backwardh(parseFloat(this.state.x), parseFloat(this.state.h), parseInt(this.state.x1))}
                                ...
                                <Col style={{ width: 400 }} span={12}>
                                    <Card style={{ width: 350, padding: '3px', background: '#91d5ff' }} title="Eqation from Database" bordered={false}>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => this.backwardh(parseFloat(this.state.data[5].x), parseFloat(this.state.data[5].h), parseFloat(this.state.data[5].x1))}
